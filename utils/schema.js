...
export const ACTIVITIES = mysqlTable('activities', {
  id: int('id').primaryKey().autoincrement(),
  course_id: int('course_id').references(() => COURSES.id).notNull(), // Foreign key referencing the course
...
export const USER_ACTIVITIES = mysqlTable('user_activities', {
  ...
  course_id: int('course_id').references(() => COURSES.id).notNull(),
  activity_id: int('activity_id').references(() => ACTIVITIES.id).notNull(),
...
export const LEARN_TOPICS = mysqlTable('learn_topics', {
  id: int('id').primaryKey().autoincrement(),
  learn_id: int('learn_id').references(() => LEARN.id).notNull(), // Foreign key to 'learn' table
  title: varchar('title', { length: 255 }).notNull(),
  slug: text('slug').notNull().default(''),
  image: varchar('image', { length: 255 }), // Stores the URL or path for the image
...
export const OPTIONS = mysqlTable('options', {
  id: int('id').primaryKey().autoincrement(),
  question_id: int('question_id').references(() => QUESTIONS.id).notNull(), // Foreign key to 'questions' table
  learn_topic_id: int('learn_topic_id').references(() => LEARN_TOPICS.id).notNull(), // Foreign key to 'learn_topics' for redundancy
...
export const USER_PROGRESS = mysqlTable('user_progress', {
  id: int('id').primaryKey().autoincrement(),
  user_id: int('user_id').references(() => USER_DETAILS.id).notNull(), // Foreign key referencing the user
  child_id: int('child_id').references(() => CHILDREN.id).notNull(), // Foreign key referencing the child
  question_id: int('question_id').references(() => QUESTIONS.id).notNull(), // Foreign key referencing the question
...
export const BADGES = mysqlTable('badges', {
  id: int('id').primaryKey().autoincrement(),
  child_id: int('child_id').references(() => CHILDREN.id).notNull(), // Foreign key to children
  badge_type: mysqlEnum('badge_type', ['search', 'quiz', 'achievement']).notNull(),
  title: varchar('title', { length: 255 }).notNull(),
  image: varchar('image', { length: 255 }),
...
export const USER_BADGES = mysqlTable('user_badges', {
  ...
  child_id: int('child_id').references(() => CHILDREN.id).notNull(), // Foreign key referencing the child
  badge_id: int('badge_id').references(() => BADGES.id).notNull(), // Foreign key referencing the badge
